...
//                    int wordSlotId = Integer.valueOf(String.valueOf(slotId));
        ...
        System.out.println("chatSlot2Fill---------------->" + chatSlot2Fill);
    ...
    public ChatVo chat2(int robotId, String chatMsg,String userId,String token,JSONObject data) {
        ...
        String intentionNumber = session.getCurrentIntentionNumber();
        ChatWordSlot chatSlot2Fill = session.getChatWordSlot2Fill();
        String chatSlotNumber = chatSlot2Fill == null ? null : chatSlot2Fill.getNumber();
        JSONObject dataJson = nlpService.getIntent(robotId, chatMsg, intentionNumber, chatSlotNumber);
...
//            chatVo.setMsg("当前没有获得意图");
        ...
        List<ChatWordSlot> slotList = chatIntention.getChatSlotList();
        if (null != slotList && slotList.size() != 0) {
            for (ChatWordSlot chatSlot : slotList) {
                SLOT_STATE slotState = SLOT_STATE.UN_FILL;
                List<String> processedList = new ArrayList<>();
        ...
        if(chatVo.getIntention().getNumber().equals("water_level")){
            ...
            //遍历词槽
            chatVo.getIntention().getChatSlotList().forEach(chatSlot -> {
                //第一个词槽存站点名
                if(map.size() == 0){
